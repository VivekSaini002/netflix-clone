...
import com.netflixClone.exception.InvalidRoleException;
...
public class UserServiceImpl implements UserService {
	...
	public MessageResponse createUser(UserRequest userRequest) {
		...
		return new MessageResponse("User created successfully. Verification email sent to " + userRequest.getEmail());
	...
	public MessageResponse deleteUser(Long id, String currentUserEmail) {
		...
		if(user.getEmail().equals(currentUserEmail)) {
			throw new RuntimeException("Users cannot delete your own accounts.");
		}
	...
	private void ensureNotLastAdmin(User user, String operation) {
		if(user.getRole() == Role.ADMIN) {
			long activeAdminCount = userRepository.countByRole(Role.ADMIN);
			if(activeAdminCount <= 1) {
